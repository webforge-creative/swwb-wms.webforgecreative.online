...
class RestController extends \CI_Controller
{
	...
	public function __construct($config = 'rest')
	{
		...
		if (!empty($this->config->item('app_key'))) {
			$this->is_valid_request = $this->_detect_app_key();
		}
	...
	public function _remap($object_called, $arguments = [])
	{
		...
		try {
			if ($this->is_valid_request) {
				...
				$response = [$this->config->item('rest_status_field_name') => false, $this->config->item('rest_message_field_name') => "Invalid Request"];
				$this->response($response, self::HTTP_UNAUTHORIZED);
			}
		} catch (Exception $ex) {
	 ...
	 * @return bool
	 */
	protected function _detect_app_key()
	{
		// Get the api key name variable set in the rest config file
		$api_key_variable = "APP-KEY";

		// Work out the name of the SERVER entry based on config
		$key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
		if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
			if (!($key === $this->config->item('app_key'))) {
				return false;
			} else {
				return true;
			}
