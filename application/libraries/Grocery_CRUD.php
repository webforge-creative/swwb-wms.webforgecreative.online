		...
		switch ($real_type) {
			...
			case 'upload_file':
				if (empty($value)) {
					...
					// $file_url_anchor = '<a href="#"';
					if ($is_image) {
						$file_url_anchor .= ' class="image-popup"><img src="' . $file_url . '" height="50px">'; // image-thumbnail
						// $file_url_anchor .= ' data-image="'. $file_url .'" class="image-thumbnail"><img src="' . $file_url . '" height="50px">';
					} else {
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function get_total_results()
	{
		...
		if (!empty($this->or_where)) foreach ($this->or_where as $or_where)
				$this->basic_model->or_where($or_where[0], $or_where[1], $or_where[2]);

		if (!empty($this->like)) foreach ($this->like as $like)
				$this->basic_model->like($like[0], $like[1], $like[2]);

		if (!empty($this->or_like)) foreach ($this->or_like as $or_like)
				$this->basic_model->or_like($or_like[0], $or_like[1], $or_like[2]);

		if (!empty($this->having)) foreach ($this->having as $having)
				$this->basic_model->having($having[0], $having[1], $having[2]);

		if (!empty($this->or_having)) foreach ($this->or_having as $or_having)
				$this->basic_model->or_having($or_having[0], $or_having[1], $or_having[2]);

		if (!empty($this->relation)) foreach ($this->relation as $relation)
	...
	protected function set_ajax_list_queries($state_info = null)
	{
		...
		if (isset($state_info->search) && $state_info->search !== '') {
			...
			// Search all field
				...
				$temp_where_query_array = [];
				$basic_table = $this->get_table(); foreach ($columns as $column) {
	...
	protected function db_insert_validation()
	{
		...
		if (!empty($required_fields)) {
			foreach ($add_fields as $add_field) {
				...
				if (isset($this->set_minlength[$field_name])) {
					$min = '|min_length[' . $this->set_minlength[$field_name] . ']';
				} else {
					$min = '';
				}

				if (isset($this->set_maxlength[$field_name])) {
					$max = '|max_length[' . $this->set_maxlength[$field_name] . ']';
				} else {
					$max = '';
				}

				if (isset($this->set_pattren[$field_name])) {
					$pattren = '|regex_match[' . $this->set_pattren[$field_name] . ']';
				} else {
					$pattren = '';
				}
				...
				if (array_key_exists($field_name, $this->relation_n_n) && in_array($field_name, $required_fields)) {
					if (!array_key_exists($field_name, $_POST)) {
						...
						$this->set_rules($field_name, $field_types[$field_name]->display_as, 'required' . $min . $max . $pattren);
					}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
		...
		foreach ($list as $num_row => $row) {
			...
			foreach ($this->actions as $unique_id => $action) {
				if (!empty($action->url_callback)) {
					...
					if($action->link_url == '#'){
						$actions_urls[$unique_id] = $action->link_url;
					} else {
	...
	protected function get_dropdown_input($field_info, $value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='select2' data-placeholder='" . $select_title . "'>";
	...
	protected function get_set_input($field_info, $value)
	{
		...
		$select_title = str_replace('{field_display_as}', $field_info->display_as, $this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='select2' data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		if (empty($unselected_values) && empty($selected_values)) {
			...
			$css_class = $has_priority_field || $is_ie_7 ? 'multiselect' : 'select2';
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$image_class = $is_image ? 'image-popup' : ''; // image-thumbnail

		$input = '<span class="fileinput-button qq-upload-button btn btn-primary" id="upload-button-' . $unique . '" style="' . $uploader_display_none . '">
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	const JQUERY_UI_JS = "jquery-ui-1.10.3.custom.min.js?l=l";
	...
	public function set_column($field_name, $set_column = null)
	{
		if (is_array($field_name)) {
			foreach ($field_name as $field => $set_column) {
				$this->set_column[$field] = $set_column;
			}
		} elseif ($set_column !== null) {
			$this->set_column[$field_name] = $set_column;
		}
		return $this;
	}

	// Minimum Length of Field
		public function set_minlength($field_name, $set_minlength = null)
		{
			if (is_array($field_name)) {
				foreach ($field_name as $field => $set_minlength) {
					$this->set_minlength[$field] = $set_minlength;
				}
			} elseif ($set_minlength !== null) {
				$this->set_minlength[$field_name] = $set_minlength;
			}
			return $this;
		}
	// End...
	...
	// Maximum Length of Field
		public function set_maxlength($field_name, $set_maxlength = null)
		{
			if (is_array($field_name)) {
				foreach ($field_name as $field => $set_maxlength) {
					$this->set_maxlength[$field] = $set_maxlength;
				}
			} elseif ($set_maxlength !== null) {
				$this->set_maxlength[$field_name] = $set_maxlength;
			}
			return $this;
		}
	// End...

	// Set Pattren of Field
		public function set_pattren($field_name, $set_pattren = null)
		{
			if (is_array($field_name)) {
				foreach ($field_name as $field => $set_pattren) {
					$this->set_pattren[$field] = $set_pattren;
				}
			} elseif ($set_pattren !== null) {
				$this->set_pattren[$field_name] = $set_pattren;
			}
			return $this;
		}
	// End...
			...
			if (!empty($this->add_fields)) {
				...
				foreach ($field_types as $field) {
					...
					if ((!isset($field->db_extra) || $field->db_extra != 'auto_increment')) {
						...
						// Custom
						if(isset($this->display_as[$field->name])){
							$d = $this->display_as[$field->name];
						} else {
							$d = $field->display_as;
						}

						if(isset($this->set_column[$field->name])){
							$c = $this->set_column[$field->name];
						} else {
							$c = 'col-md-6';
						}

						$this->add_fields[] = (object) array('field_name' => $field->name, 'display_as' => $d, 'set_column' => $c);
					}
				}
			}
			...
			if (!empty($this->edit_fields)) {
				...
				foreach ($field_types as $field) {
					...
					if (!isset($field->db_extra) || $field->db_extra != 'auto_increment') {
						...
						$this->edit_fields[] = (object) array('field_name' => $field->name, 'display_as' => $d, 'set_column' => $c);
					}
				}
			}
