...
class Grocery_crud_model  extends CI_Model  {
	...
	// Activity Logs
		protected function log_activity($action, $table, $data) {
			$log_data = array(
				'log_activity_action' => $action,
				'log_activity_table_name' => $table,
				'log_activity_data' => json_encode($data),
				'log_activity_by' => $this->session->userdata('user_id'), // Assuming user ID is stored in session
				'log_activity_date' => date('Y-m-d H:i:s')
			);
			$this->db->insert('log_activies', $log_data);
		}

		public function insert_record($table, $data) {
			$this->db->insert($table, $data);
			if ($this->db->affected_rows() > 0) {
				$this->log_activity('insert', $table, $data);
		...
		public function update_record($table, $data, $condition) {
			...
			$data[array_keys($condition)[0]] = array_values($condition)[0];
			if ($this->db->affected_rows() > 0) {
				$this->log_activity('update', $table, $data);
				return true;
			}
			return false;
		}
		...
		public function delete_record($table, $condition) {
			...
			if ($this->db->affected_rows() > 0) {
				$this->log_activity('delete', $table, $data);
				return true;
			}
			return false;
		}
